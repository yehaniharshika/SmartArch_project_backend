...
def _crop_and_read(img, x1, y1, x2, y2, reader, det_class):
    ...
    # nothing usable for this crop. Labels are already reliable (per
    # your logs — HALL/DINING/BED/KITCHEN all extracted correctly), so
    # there's no need to spend a Gemini call on every label crop.
    ...
    # not just empty ones, because dimension misreads are the main
    # accuracy problem even when EasyOCR *did* return something (e.g.
    # "8'X10'" misread as "8XI0'"). Gemini's reading REPLACES the
    # EasyOCR item(s) for this crop when it parses into a valid,
    # non-zero dimension; otherwise the original EasyOCR reading (if
    # any) is kept as-is.
    elif det_class == "dimension_text":
